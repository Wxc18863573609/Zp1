...
import static com.zupu.zp.fragment.Livefragment.mCommentsAdapter;
import static com.zupu.zp.fragment.Livefragment.zbrecycouview1;
...
public class JoinLiveAnchorUI extends BaseActivitys {
   ...
   // private String anchorStreamID = ZegoUtil.getPublishStreamID();
    ...
    protected void initViewList(){
        ...
        mBigView = new JoinLiveView(binding.previews, false, "");
        ...
        ZGJoinLiveHelper.sharedInstance().getZegoLiveRoom().loginRoom(mRoomID, ZegoConstants.RoomRole.Anchor, new IZegoLoginCompletionCallback() {
            ...
            public void onLoginCompletion(int errorCode, ZegoStreamInfo[] zegoStreamInfos) {
                ...
                if (errorCode == 0) {
                    ...
                    Toast.makeText(JoinLiveAnchorUI.this, getString(R.string.tx_login_room_failure), Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
    ...
    public void initSDKCallback(){
        ...
        ZGJoinLiveHelper.sharedInstance().getZegoLiveRoom().setZegoLivePlayerCallback(new IZegoLivePlayerCallback() {
            ...
            public void onPlayStateUpdate(int stateCode, String streamID) {
                ...
                if (stateCode == 0) {
                    ...
                    Toast.makeText(JoinLiveAnchorUI.this, getString(R.string.tx_play_success), Toast.LENGTH_SHORT).show();
                } else {
                    AppLogger.getInstance().i(JoinLiveAnchorUI.class, "拉流失败, streamID : %s, errorCode : %d", streamID, stateCode);
                    Toast.makeText(JoinLiveAnchorUI.this, getString(R.string.tx_play_fail), Toast.LENGTH_SHORT).show();
        ...
        ZGJoinLiveHelper.sharedInstance().getZegoLiveRoom().setZegoIMCallback(new IZegoIMCallback() {
            ...
            public void onRecvRoomMessage(String s, ZegoRoomMessage[] zegoRoomMessages) {
                ...
                Toast.makeText(JoinLiveAnchorUI.this, "11", Toast.LENGTH_SHORT).show();
            }
     ...
     * 房间聊天消息.
     */
    public void handleRecvRoomMsg(String roomID, ZegoRoomMessage[] listMsg) {

        zbrecycouview1.setLayoutManager(new LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false));
        zbrecycouview1.setAdapter(mCommentsAdapter);
        // 设置Item添加和移除的动画
        zbrecycouview1.setItemAnimator(new DefaultItemAnimator());

        for (ZegoRoomMessage message : listMsg) {
            Toast.makeText(this, "执行监听", Toast.LENGTH_SHORT).show();
            // 文字聊天消息
            if (message.messageType == ZegoIM.MessageType.Text && message.messageCategory == ZegoIM.MessageCategory.Chat) {
                ...
                if (listTextMsg.size() > 0) {
                    ...
                    zbrecycouview1.scrollToPosition(mCommentsAdapter.getListMsg().size() - 1);
                    Log.e("TAG", "可以执行" );
                    zbrecycouview1.post(new Runnable() {
                        @Override
                        public void run() {
