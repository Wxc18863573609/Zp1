...
import com.zupu.zp.databinding.ActivityPublishBinding;
...
import com.zupu.zp.utliss.InitSDKutil;
...
public class PublishActivityUI extends BaseActivitys {
    ImageView fx;
  ...
  //  private SDKConfigInfo sdkConfigInfo = new SDKConfigInfo();
    ...
    RelativeLayout camera,pl,beauty,filter;
    Httputlis1 instance1 = Httputlis1.getInstance();
    String title,status,cover,proIds, uuid1;;
    int isPK=0,activityId;
    //ArrayList<Integer> list = new ArrayList<>();
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = DataBindingUtil.setContentView(this, activity_publish);
        ...
        // list=(ArrayList)intent.getSerializableExtra("ids");
        // list=(ArrayList)intent.getSerializableExtra("ids");
     ...
     //获取开始时间
        ...
        starttime=simpleDateFormat.format(date);
         ...
         streamID=intent.getStringExtra("starmID");
       ...
       //监听软键盘关闭
        ...
        kill.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                new AlertDialog.Builder( PublishActivityUI.this)
        ...
        fx.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                zfUtil.myshare(UrlCount.Base_Head+"mobile/live_share.html?roomId="+roomnum+"&streamId="+streamID+"&isPlayBack="+0+"&isPK="+0+"&playUrl="+"null","直播分享","有人向你分享了直播",sharedPreferences.getString("photoUrl",null),PublishActivityUI.this);
            }
        });
      ...
      /*   //发送邀请连麦
         send.setOnClickListener(new View.OnClickListener() {
             ...
             public void onClick(View v) {
                 doSendRoomMsg("11");
                 ZGBaseHelper.sharedInstance().getZegoLiveRoom().inviteJoinLive("1d88d643a2e0080d-14", new IZegoResponseCallback() {
                   @Override
                   public void onResponse(int i, String s, String s1) {

                       Log.e("TAG", "onResponse: 结果"+i);
                       if (i==0){
                           Toast.makeText(PublishActivityUI.this, "同意", Toast.LENGTH_SHORT).show();
                       }else {
                           Toast.makeText(PublishActivityUI.this, "拒绝", Toast.LENGTH_SHORT).show();
                       }
                   }
               });
             }
    ...
    // ZGJoinLiveHelper.sharedInstance().getZegoLiveRoom().startPublishing(anchorStreamID, "anchor", ZegoConstants.PublishFlag.JoinPublish);
     /*               if (!isPublishSuccessd) {
        AppLogger.getInstance().i(ZGPublishHelper.class, "推流失败啊锕, streamID : %s", streamID);
        Toast.makeText(PublishActivityUI.this,"推流失败啊锕", Toast.LENGTH_SHORT).show();
        // 修改标题状态拉流失败状态
        ...
        instance1.posthttps(UrlCount.Base_CreatRoom, map, new Httputlis1.Mycallbacks() {
            ...
            public void sucess(String json) {
                ...
                if (creatRoomBean.getCode()==0){
                    streamID=creatRoomBean.getEntity().getExtPara3()+"";
                    hideInputStreamIDLayout();
                    ...
                    Toast.makeText(PublishActivityUI.this, "开播失败", Toast.LENGTH_SHORT).show();
                }
            }
        });
   ...
   /*     view2.textureView.setOnClickListener(new View.OnClickListener() {
    ...
    public void finish() {
      ...
      //  getremoveUser();
       ...
       // getremoveUser();
        ...
        Intent intent = new Intent(PublishActivityUI.this,EndLiveActivity.class);
    ...
    public void run2(ZegoStreamInfo[] zegoStreamInfos){
     ...
     //   ZGJoinLiveHelper.sharedInstance().getZegoLiveRoom().startPublishing(anchorStreamID, "anchor", ZegoConstants.PublishFlag.JoinPublish);
        ...
        ZGPublishHelper.sharedInstance().setPublisherCallback(new IZegoLivePublisherCallback() {
            ...
            public void onPublishStateUpdate(int errorCode, String streamID, HashMap<String, Object> hashMap) {
                ...
                if (errorCode == 0) {
                    ...
                    Toast.makeText(PublishActivityUI.this, "前面失败", Toast.LENGTH_SHORT).show();
                    // 当推流失败时需要显示布局
                    showInputStreamIDLayout();
                }
            ...
            public void onJoinLiveRequest(int i, String s, String s1, String s2) {
                ...
                Log.e("TAG", "收到连麦请求" );
                Toast.makeText(PublishActivityUI.this, R.string.sdlmqq, Toast.LENGTH_SHORT).show();
        ...
        hideInputStreamIDLayout();
        boolean isPublishSuccessd = ZGPublishHelper.sharedInstance().startPublishing(streamID, "", ZegoConstants.PublishFlag.JoinPublish);
    }
       ...
       /* View views= View.inflate(context, R.layout.activity_publish, null);
        ...
        boolean results = ZGBaseHelper.sharedInstance().initZegoSDK(null,giftParent,mContext,ZegoUtil.getAppID(), ZegoUtil.getAppSign(), ZegoUtil.getIsTestEnv(), new IZegoInitSDKCompletionCallback() {
            ...
            public void onInitSDK(int errorCode) {
                // 关闭加载对话框
                CustomDialog.createDialog(PublishActivityUI.this).cancel();
                // errorCode 非0 代表初始化sdk失败
                // 具体错误码说明请查看<a> https://doc.zego.im/CN/308.html </a>
                if (errorCode == 0) {
                    AppLogger.getInstance().i(this.getClass(), "初始化zegoSDK成功");
                    Toast.makeText(PublishActivityUI.this, "初始化成功", Toast.LENGTH_SHORT).show();
                    ...
                    AppLogger.getInstance().i(PublishActivityUI.this.getClass(), "初始化sdk失败 错误码 : %d", errorCode);
                    Toast.makeText(PublishActivityUI.this, "初始化失败", Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    private void loginRoomCompletion(boolean isLoginRoom, String finalRoomId, int errorCode,String steamID) {
        ...
        if (isLoginRoom) {
            //Toast.makeText(this, "登陆房间成功", Toast.LENGTH_SHORT).show();
            ...
            Toast.makeText(PublishActivityUI.this, "登陆房间失败", Toast.LENGTH_SHORT).show();
        }
    }
    //接收数据
                ...
                if ("1".equals(mRoomID)){
                    ...
                    for (ZegoStreamInfo streamInfo : zegoStreamInfos) {
                        ...
                        // 当有其他流关闭的时候，停止拉流
                            ...
                            AppLogger.getInstance().i(PublishActivityUI.class, "房间：%s 内收到流删除通知. streamID : %s, userName : %s, extraInfo : %s", "1", streamInfo.streamID, streamInfo.userName, streamInfo.extraInfo);
    ...
    public void getdate() {
        ...
        Glide.with(PublishActivityUI.this).load( sharedPreferences.getString("photoUrl",null))
